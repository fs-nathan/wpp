...
const DrawerQuickAccess = (props) => {
  ...
  const handleUnPinBoard = () => {
    ...
    CustomEventEmitterWithParams("WPS_HIDE_WORKING_START_BUTTON", {
      detail: { isHide: !isHideStartButton },
    });
  };
            ...
            <div style={{ padding: "0 18px" }}>
              <div className={"quickAccess--groupContent"}>
                ...
                <div className={"quickAccess--groupContent-body"}>
                  <List
                    component="nav"
                    aria-label="main"
                    className={"quickAccess--groupContent-listCustom"}
                  >
                    {map(recentlyProjects, function (project) {
                      return (
                        ...
                        >
                          <ListItemIcon
                            className={"quickAccess--groupContent-iconCustom"}
                          >
              ...
              <div className={"quickAccess--groupContent"}>
                <div className={"quickAccess--groupContent-header"}>
                  ...
                  <Typography component="h4">
                    {t("LABEL_PERSONAL_BOARD")}
                  </Typography>
                  <div
                    className={"quickAccess--groupContent-header__controlLeft"}
                  >
                    <Link
                      component={"button"}
                      onClick={() => {
                ...
                <div className={"textSecondary"}>
                  {t("PROJECT_SETTING_MODAL_PANEL_PIN_DESCRIPTION2")}
                </div>
                <List
                  component={"nav"}
                  className={"quickAccess--groupContent-listCustom"}
                              ...
                              >
                                {(provided, snapshot) => (
                                  <ListItem
                                    ...
                                    className={
                                      "quickAccess--groupContent-itemCustom"
                                    }
                                    onClick={() => {
                                  ...
                                  >
                                    <ListItemIcon
                                      className={
                                        "quickAccess--groupContent-iconCustom"
                                      }
                                    >
                                      {/* <img
                                        src={resolvedWorkType(
                                          project.work_type
                                        )}
                                        alt=""
                                        width={22}
                                    ...
                                    <div
                                      className={
                                        "quickAccess--groupContent-listCustom-closeIcon"
                                      }
                                    >
                                      <IconButton
                                        size={"small"}
                                        onClick={(e) =>
                                          handleDelete(e, get(project, "id"))
                                        }
                                      >
                                        <Icon
                                          path={mdiClose}
              ...
              <div className={"quickAccess--bottomLink"}>
                <Link onClick={() => handleUnPinBoard()}>
                  {isHideStartButton
                    ? t("LABEL_VISIBLE_PERSONAL_BOARD_FROM_WORK_CATEGORY")
                    : t("LABEL_HIDE_PERSONAL_BOARD_FROM_WORK_CATEGORY")}
                </Link>
                <Link onClick={() => handleCreateNewWorkingBoard()}>
                  {t("LABEL_CREATE_WORK_TOPIC_PROJECT_PROCESS")}
                </Link>
              </div>
            </div>
          </Scrollbars>
