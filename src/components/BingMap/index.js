...
class ReactBingmaps extends Component {
	...
	componentWillReceiveProps(nextProps){
		...
		if(this.props.pushPinsHtml !== nextProps.pushPinsHtml){
			this.setPushPinsHtml(nextProps.pushPinsHtml, mapReference);
		}
	...
	reactBingmaps(props,  Microsoft){
		...
		if(bingmapKey && Microsoft){
			...
			this.setPushPinsHtml(pushPinsHtml, mapReference);
	...
	setPushPinsHtml(htmlContents, mapReference){
		if(map[mapReference] && htmlContents){
			Microsoft.Maps.registerModule('HtmlPushpinLayerModule', '/lib/HtmlPushpinLayerModule.js');
			Microsoft.Maps.loadModule('HtmlPushpinLayerModule', function (e) {
				var points = htmlContents.map(p => {
					return new window.HtmlPushpin(new Microsoft.Maps.Location(p.location.latitude, p.location.longitude), p.html, new Microsoft.Maps.Point(36, 60))
					// return new window.HtmlPushpin(new Microsoft.Maps.Location(p.location.latitude, p.location.longitude), "<div style='background: red; width: 5px; height: 5px; border-radius: 50%;'></div>", new Microsoft.Maps.Point(2.5, 2.5))
				})

	            var layer = new window.HtmlPushpinLayer();
	            layer.setPushpins(points);

	            //Add the HTML pushpin to the map.
	            map[mapReference].layers.insert(layer);
	        });
		}
	}
