...
const ModalContinueCreateAccount = ({
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const newUserCreate = rowTable.map(e => {
                return {
                    email: e.email,
                    name: e.name,
                    room: e.room ? e.room : roomList[0].id
                }
            })
            const { data } = await actionAddMutipleMember({
                account_list: newUserCreate,
    ...
    return (
        <CustomModal
            ...
            className={`modal_continue-create ${
                loading && "modal_continue-create-loading"
            }`}
        >
            <form id="form-create-multile-account" onSubmit={handleSubmit}>
                ...
                <div className="modal_continue-create_table">
                    <table style={{ borderCollapse: "collapse" }}>
                        <thead>
                            <tr>
                                ...
                                <th>
                                    {t("IDS_WP_ACCOUNT_INTERNAL_TABLE_ACCOUNT")}
                                </th>
                                <th>
                                    {t(
                                        "IDS_WP_ACCOUNT_INTERNAL_TABLE_NAME_MEMBER"
                                    )}
                                </th>
                                <th>
                                    {t("IDS_WP_ACCOUNT_INTERNAL_TABLE_PART")}
                                </th>
                                <th>
                                    <div
                                        style={{
                                    ...
                                    >
                                        {t(
                                            "IDS_WP_ACCOUNT_INTERNAL_TABLE_DELETE_ALL"
                                        )}
                                    </div>
                                </th>
                            </tr>
                                    ...
                                    <tr key={`${index}`}>
                                        ...
                                        <td>
                                            <input
                                                ...
                                                type="email"
                                                placeholder={t(
                                                    "IDS_WP_CREATE_ACCOUNT_ENTER_EMAIL"
                                                )}
                                        ...
                                        <td>
                                            <input
                                                ...
                                                required
                                                placeholder={t(
                                                    "IDS_WP_CREATE_ACCOUNT_ENTER_NAME"
                                                )}
                    ...
                    <p style={{ color: "red" }}>
                        {t("IDS_WP_ACCOUNT_INTERNAL_ADDED")}: {rowTable.length}
                        /100 {t("IDS_WP_ACCOUNT_INTERNAL_ADDED_NOTE")}
                    </p>
                    <h5>
                    ...
                    >
                        {ReactParserHtml(
                            t("IDS_WP_ACCOUNT_INTERNAL_SET_PASSWORD")
                        )}
                    </div>
                    <FormControl
                        margin="normal"
                ...
                {loading && (
                    <div className="loading">
                        ...
                        <div style={{ color: bgColor.color, fontSize: "16px" }}>
                            {t("IDS_WP_CREATING_ACOUNT")}
                        </div>
                        <div style={{ marginTop: "5px" }}>
                            {t("IDS_WP_CREATING_ACCOUNT_TIME_OUT")}
                        </div>
                    </div>
                )}
            </form>
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        doSearchUser: ({ info }, quite) =>
            dispatch(searchUser({ info }, quite)),
