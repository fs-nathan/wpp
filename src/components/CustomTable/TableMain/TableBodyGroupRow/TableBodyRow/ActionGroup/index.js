...
export const ActionList = ({ index, row, group }) => {
    ...
    const { update_task, delete_task, status_label } = useSelector((state) =>
        get(state, "taskDetail.detailTask.taskDetails.permissions", {})
    );
    const { is_ghim: isPinned, state_code, assign_code, is_stop } = taskDetails || {};
    const pause = is_stop;
            ...
            <div>
                ...
                {openAdd && taskDetails.project && (
                    <AddMemberModal
                        ...
                        onloadAddMember={onloadAddMember}
                ...
                {taskDetails && taskId && Boolean(anchorEl) && (
                    <ProgressModal
                        setOnload={setOnload}
