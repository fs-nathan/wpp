...
function SimpleDonutChart({ percentDone = 0, color = '#31b586', variant = 'normal' }) {
  ...
  return (
    <svg height={size} width={size}>
      <circle cx={size / 2} cy={size / 2} r={size / 2 - 5} stroke="#e0e0e0" strokeWidth="4" fill='none'/>
      <path d={describeArc(size / 2, size / 2, size / 2 - 5, 0, deg)} stroke={color} strokeWidth="4" fill='none'/>
