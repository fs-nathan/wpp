...
const ModalSettingMember = ({ open, setOpen, data ,updatedUser = null,
    options,
    doUpdateUser,
    doListLevel,
    doListMajor,
 ...
 React.useEffect(()=> {
  ...
  return (
    <CustomModal
      ...
      cancleRender={tab === "2" | tab === "3" && (()=> (t('LABEL_CHAT_TASK_THOAT')))}
    ...
    >
        ...
        <div className="modalSettingMember">
          <div className="modalSettingMember-tab-left">
            ...
            >
              ...
              <h5>{profile.name}</h5>
              <p>{profile.level || 'Chuyên viên'}</p>
              <p>{t('IDS_WP_SPECIES')}: <span style={{color: 'red'}}>{profile.role}</span></p>
              <p>{t("DMH.VIEW.DP.RIGHT.UT.STATE.TITLE")}: <span style={{color: 'green'}}>hoạt động</span></p>
              <p>
                {t("views.calendar_page.right_part.label.created_by")}: Trần
                cung
              </p>
              <p>
            ...
            <div className="modalSettingMember-tab-left_menu">
              <div className={`modalSettingMember-tab-left_menu-item ${tab === '1' && 'modalSettingMember-tab-left_menu-item_active'}`} onClick={()=>handleChangeTab('1')}>
                <div>
                  <Icon
                    path={mdiAccountCog}
                    size={1}
              ...
              <div className={`modalSettingMember-tab-left_menu-item ${tab === '2' && 'modalSettingMember-tab-left_menu-item_active'}`} onClick={()=>handleChangeTab('2')}>
                <div>
                  <IconAccountInternal />
                </div>
                <div>{t("IDS_WP_ACCOUNT_INTERNAL")}</div>
              </div>
              <div className={`modalSettingMember-tab-left_menu-item ${tab === '3' && 'modalSettingMember-tab-left_menu-item_active'}`} onClick={()=>handleChangeTab('3')}>
                <div>
      ...
      />
                ...
                <div className="modalSettingMember-right_account-internal--note">{t('IDS_WP_RESET_PASSWORD_NOTE')}</div>
                <form>
                <FormControl
              margin="normal"
              variant="outlined"
            ...
            <div style={{fontSize: '14px', marginTop: '20px'}}>{t('IDS_WP_PASSWORD_VALID_DES')}</div>
            <Button className="modalSettingMember-right_account-internal--btn_reset" type="submit">{t('IDS_WP_RESET')}</Button>
                </form>
            </div>
            ) : (
              <div className="modalSettingMember-right_account-internal">
                 <h5>{t('IDS_WP_LOCK_MEMBER')}</h5>
                 <div className="modalSettingMember-right_account-internal--note">{t('IDS_WP_LOCK_MEMBER_FOREVER')}</div>
                 <Button className="modalSettingMember-right_account-internal--btn_reset">{t('IDS_WP_LOCk')}</Button>
              </div>
            )}
          </div>
        </div>
