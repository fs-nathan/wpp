...
import EditJobModal, {
    EDIT_MODE,
} from "../../../ListPart/ListHeader/CreateJobModal";
import { taskIdSelector } from "../../../selectors";
import "./styles.scss";
import ProgressModal from "views/Chat/TabPart/ProgressTab/ProgressModal";

function getAssignType(assign_code) {
    if (assign_code === 0) return "LABEL_CHAT_TASK_DA_DUOC_GIAO_NGAY";
    if (assign_code === 1) return "LABEL_CHAT_TASK_DA_DE_XUAT_NGAY";
...
function TabHeader(props) {
    ...
    const { is_ghim: isPinned, status_label, assign_code, is_stop } = taskDetails || {};
    const pause = is_stop;
    ...
    const { update_task, delete_task } = useSelector((state) =>
                ...
                {detailTask && (
                    ...
                    >
                        ...
                        {t("LABEL_CREATED_SHORT_FIXED", {
                            date: detailTask.date_create,
                            time: detailTask.created_time,
                        })}
                    </ColorTypo>
                )}
            </div>
