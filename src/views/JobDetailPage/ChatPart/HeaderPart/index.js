...
const StyledFormControlLabel = styled(FormControlLabel)`
  & .MuiTypography-root {
    background-color: ${(props) =>
      props.checked ? props.groupActiveColor : "#f2f2f2"};
      ...
      >
        {props.tabs.map((label, idx) => (
          <StyledFormControlLabel
            className={clsx("chatHeader--tabLabel", {
              "chatHeader--tabLabel__selected": value === label,
            })}
            key={idx}
            value={label}
            control={<Radio />}
  ...
  return (
    <div className="wrap-avatars">
      {showImages.map(({ avatar }, i) => (
        <Avatar
          key={i}
          className={clsx(`chatHeader--avatar${showImages.length}_${i + 1}`, {
            [`chatHeader--avatar${showImages.length}_${i + 1}_plus`]:
              plusImage > 0,
          })}
          src={avatar}
        />
      ...
      {plusImage > 0 && (
        <Avatar className="header-chat-avatar chatHeader--avatar_plus">{`+${
          plusImage < 100 ? plusImage : 99
        }`}</Avatar>
      )}
    </div>
  );
...
const HeaderPart = (props) => {
  ...
  const members = useSelector(
    (state) => state.taskDetail.taskMember.defaultMember
  );
  useEffect(() => {
