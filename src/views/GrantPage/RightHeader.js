...
const menuuu = <Menu style={{zIndex: 1}}>
<Menu.Item  >
  ...
  <td>
    ...
    Lịch làm ca tối của thầu phụ
      ...
      <CustomModal
            ...
            height='tall'
            confirmRender={() => <div style={{color: 'blue', fontSize: 16}}>Chỉnh sửa</div>}
            columns={2}
            left = {{
          ...
          />
  <Menu>
    <Menu.Item onClick={clickConfigCalendar} >
      <p className="right-header-more">
      Lịch dự án
      </p>
    </Menu.Item>
    <Menu.Item onClick={() =>changeVisibleExportPdfDrawer(true)}>
      <p className="right-header-more">
        Xuất file PDF
      </p>
    </Menu.Item>
...
const RightHeader = ({
    ...
    return(
    <Row justify="end">
        <Col span={3}><IconComponent title={"TÌM KIẾM"} path={mdiMagnify}/></Col>
        <Col span={3}><IconComponent onClick={() => changeVisibleConfigGantt(true, "TIME")} title={"TIẾN ĐỘ: NGÀY"} path={mdiCalendar}/></Col>
        <Col span={3}><IconComponent onClick={() => changeShowFullChart(!showFullChart)} title={!showFullChart ? "MỞ RỘNG" : "Thu gọn"} path={!showFullChart ?mdiFullscreen : mdiFullscreenExit}/></Col>
        <Col span={3}><IconComponent onClick={() => changeVisibleConfigGantt(true, "COMMON")} title={"CÀI ĐẶT"} path={pathSettingIcon}/></Col>
        <Col style={{borderRight: '1px solid #ededed'}} span={3}>
