          ...
          <StyledListItem
            ...
            <div {...provided.dragHandleProps} style={{ marginLeft: "-10px" }}>
              <Icon
                ...
                size={1}
                color={!isHover ? "rgba(0, 0, 0, 0)" : "rgba(0, 0, 0, 1)"}
              />
            </div>
            <CustomAvatar
            ...
            />
            <ListItemText
              primary={<Primary>{get(projectGroup, "name", "")}</Primary>}
                ...
                <div className={"view_ProjectGroup_List_statistic"}>
                  {get(projectGroup, "work_types", []).map((item) => {
                    return (
                      <div
                        key={"wtl" + item}
                        className={"view_ProjectGroup_List_statistic_item"}>
                        <img
                          src={workType[parseInt(item)].image}
                          alt=''
                          width={15}
                          height={15}
                        />
                        <span>
                          {get(
                            projectGroup,
                            `statistic.${workType[parseInt(item)].key}`,
                            0
                          )}
                        </span>
                      </div>
