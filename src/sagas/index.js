...
import { listComment, listTrash, listMyDocument, listDocumentOfFolder } from './documents';
...
function* rootSaga() {
  ...
  yield takeLatest(PUBLIC_MEMBER, publicMember);
  yield takeLatest(PRIVATE_MEMBER, privateMember);
  yield takeLatest(SEARCH_USER, searchUser);
  yield takeLatest(INVITE_USER_JOIN_GROUP, inviteUserJoinGroup);
  yield takeLatest(BAN_USER_FROM_GROUP, banUserFromGroup);
  yield takeLatest(CREATE_ICON, createIcon);
  yield takeLatest(DELETE_ICON, deleteIcon);
  yield takeLatest(CREATE_PROJECT_GROUP, createProjectGroup);
  yield takeLatest(EDIT_PROJECT_GROUP, editProjectGroup);
  yield takeLatest(LIST_PROJECT_GROUP, listProjectGroup);
  yield takeLatest(DELETE_PROJECT_GROUP, deleteProjectGroup);
  yield takeLatest(SORT_PROJECT_GROUP, sortProjectGroup);
  ...
  yield takeLatest(UPDATE_STATE_JOIN_TASK, updateStateJoinTask);
